...
Only explicit human approval may change `in_review` to `completed`. After approval, change the status to `completed`, inspect the repository status and diff, stage only the implementation changes for this ticket, and create one commit following the repository's commit-message conventions. Then obtain the full commit hash and replace `Pending.` under `## Commit` in the ignored ticket with that hash. The ticket itself is never staged or committed.
