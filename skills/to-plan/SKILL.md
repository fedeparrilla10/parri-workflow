...
4. Use the resulting canonical vocabulary and existing codebase knowledge to keep terminology and decisions accurate. If codebase facts must be discovered or verified, delegate all repository searching and inspection to the `explore` subagent; do not inspect application code in the primary conversation. Use `quick` thoroughness for a concrete lookup and `medium` only when the answer requires tracing multiple files or layers. Use `very thorough` only when the user explicitly requests it.
